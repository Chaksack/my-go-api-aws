...
var ConnectionBuilder = class {
  ...
  async execute(callback) {
    ...
    const transaction = new Kysely(__spreadProps(__spreadValues({}, __privateGet(this, _props29)), {
...
var DefaultQueryCompiler = class extends OperationNodeVisitor {
  ...
  visitSelectQuery(node) {
    const wrapInParens = this.parentNode !== void 0 && !CreateViewNode.is(this.parentNode) && !UnionNode.is(this.parentNode) && !InsertQueryNode.is(this.parentNode);
    if (wrapInParens) {
      this.append("(");
    }
    if (node.with) {
...
ensureLockRowExists_fn = async function() {
  if (!await __privateMethod(this, _doesLockRowExists, doesLockRowExists_fn).call(this)) {
    try {
      await __privateGet(this, _props31).db.insertInto(MIGRATION_LOCK_TABLE).values({ id: MIGRATION_LOCK_ID }).execute();
    } catch (error) {
      if (!await __privateMethod(this, _doesLockRowExists, doesLockRowExists_fn).call(this)) {
        throw error;
      }
...
async function handler() {
  ...
  const migrator = new Migrator({
    db,
    provider: new FileMigrationProvider(import_path.default.resolve(process.env.RDS_MIGRATIONS_PATH))
  });
